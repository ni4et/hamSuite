...
const dateTimeParser = new binaryParser()
  ...
  .nest('datetime_decode', {
    ...
    formatter: function (s) {
      ...
      const gregorian_day = new Date(unixtime);
...
function encodeColor(val, obj) {
  ...
  if (color_code.length != 6 && color_code.length != 8) {
    throw new Error(
      `Invalid color code ${val}. Must be #rrggbbaa or #rrggbb format`
    );
  }
...
class WSJTXParser {
  ...
  heartbeatEncoder = new binaryEncoder()
    ...
    .uint32('revision_length', {
      encoder: (val, obj) => obj['revision'].length,
    })
  ...
  replyParser = new binaryParser()
    ...
    .uint8('low_confidence')
    .uint8('modifiers', {
      formatter: (val) => keyForValue(this.replyModifier, val),
    });
  ...
  replayEncoder = new binaryEncoder().nest(null, { type: this.baseEncoder });

  // In since v2.0
  haltTxFields = ['auto_tx_only'];
  haltTxParser = new binaryParser().uint8('auto_tx_only', {
  ...
  freeTextEncoder = new binaryEncoder()
    ...
    .uint32('text_length', {
      ...
      encoder: (val, obj) =>
        obj['text'] && obj['text'].length ? obj['text'].length : 0,
    })
    .string('text', {
      length: 'text_length',
      encoder: (val, obj) => (val ? val : ''),
    })
    .uint8('send');
  ...
  locationParser = new binaryParser().nest('location', {
    type: stringParser,
  ...
  locationEncoder = new binaryEncoder()
    .nest(null, { type: this.baseEncoder })
    .uint32('location_length', {
      encoder: (str, obj) => obj['location'].length,
    })
    .string('location', { length: 'location_length' });

  // Out since v2.0
  loggedAdifFields = ['adif_text'];
  loggedAdifParser = new binaryParser().nest('adif_text', {
    type: stringParser,
    formatter: stringFormatter,
  });

  loggedAdifEncoder = new binaryEncoder()
    .uint32('adif_text_length', {
      encoder: (str, obj) => obj['adif_text'].length,
    })
  ...
  highlightCallsignEncoder = new binaryEncoder()
    .nest(null, { type: this.baseEncoder })
    .uint32('callsign_length', {
      encoder: (str, obj) => obj['callsign'].length,
    })
  ...
  encoders = {
    ...
    highlight_callsign: [
      this.highlightCallsignEncoder,
      this.highlightCallsignFields,
    ],
  };
...
class WSJTXParser_v200 extends WSJTXParser {
  ...
  statusParser = this.statusParser.uint8('special_operation_mode', {
    formatter: (v) => keyForValue(this.specialOperationMode, v),
  });
...
class WSJTXParser_v210 extends WSJTXParser_v200 {
  // In since v2.1
  closeFields = [];
  closeEncoder = new binaryEncoder().nest(null, { type: this.baseEncoder });
  ...
  swicthConfigurationParser = new binaryParser().nest('configuration_name', {
    type: stringParser,
    formatter: stringFormatter,
  });
  ...
  swicthConfigurationEncoder = new binaryEncoder()
    .nest(null, { type: this.baseEncoder })
    .uint32('configuration_length', {
      encoder: (val, obj) => obj['configuration_name'].length,
    })
  ...
  configureEncoder = new binaryEncoder()
    ...
    .uint32('mode_length', {
      // handle special case where we can send a null string
      encoder: (val, obj) =>
        obj['mode'] && obj['mode'].length ? obj['mode'].length : 0,
    })
    .string('mode', {
      length: 'mode_length',
      encoder: (val, obj) => (val ? val : ''),
    ...
    .uint32('sub_mode_length', {
      // handle special case where we can send a null string
      encoder: (val, obj) =>
        obj['sub_mode'] && obj['sub_mode'].length ? obj['sub_mode'].length : 0,
    })
    .string('sub_mode', {
      length: 'sub_mode_length',
      encoder: (val, obj) => (val ? val : ''),
    })
    .uint8('fast_mode')
    .uint32('tr_period')
    ...
    .uint32('dx_call_length', {
      // handle special case where we can send a null string
      encoder: (val, obj) =>
        obj['dx_call'] && obj['dx_call'].length ? obj['dx_call'].length : 0,
    })
    .string('dx_call', {
      length: 'dx_call_length',
      encoder: (val, obj) => (val ? val : ''),
    })
    .uint32('dx_grid_length', {
      // handle special case where we can send a null string
      encoder: (val, obj) =>
        obj['dx_grid'] && obj['dx_grid'].length ? obj['dx_grid'].length : 0,
    })
    .string('dx_grid', {
      length: 'dx_grid_length',
      encoder: (val, obj) => (val ? val : ''),
    });
  ...
  encoders = {
    ...
    switch_configuration: [
      this.swicthConfigurationEncoder,
      this.switchConfigurationFields,
    ],
...
class WSJTXParser_v230 extends WSJTXParser_v210 {
  statusParser = this.statusParser.nest('tx_message', {
    type: stringParser,
    formatter: stringFormatter,
  });
  ...
  qsoLoggedParser = this.qsoLoggedParser.nest('adif_propogation_mode', {
    type: stringParser,
    formatter: stringFormatter,
  });
...
class JTDXParser extends WSJTXParser_v210 {
  statusParser = this.statusParser.uint8('tx_first', {
    formatter: boolFormatter,
  });
...
function decode_exchange(message) {
  ...
  if (exchange[2].startsWith('R')) {
    return {
      ...
      dx_call: exchange[0],
      de_call: exchange[1],
      snr: parseInt(exchange[2].slice(1)),
    };
  }
